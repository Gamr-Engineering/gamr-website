    ...
    <div className="w-full border border-white/10 rounded-3xl overflow-hidden bg-black/40 backdrop-blur-2xl shadow-2xl relative group">
      ...
      <div className="bg-gray-900/70 border-b border-white/10 sticky top-0 z-20 backdrop-blur-xl">
        ...
        <div className="hidden md:flex flex-col">
          ...
          <div className="flex items-center gap-4 p-2 border-b border-white/5">
            <div className="flex flex-col items-center border-r border-white/10 pr-4">
              <div className="flex items-center gap-1">
                <Button type="button" variant="ghost" size="sm" onClick={() => editor.chain().focus().undo().run()} disabled={!editor.can().undo()} className="w-8 h-8 p-0 text-gray-400"><Undo className="w-4 h-4" /></Button>
                <Button type="button" variant="ghost" size="sm" onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().redo()} className="w-8 h-8 p-0 text-gray-400"><Redo className="w-4 h-4" /></Button>
              </div>
              <span className="text-[9px] uppercase font-bold text-gray-600 tracking-tighter mt-1">History</span>
            </div>
            ...
            <div className="flex flex-col items-center border-r border-white/10 pr-4">
              <div className="flex items-center gap-4">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 gap-2 bg-white/5 text-[11px] text-gray-300 font-bold uppercase tracking-widest px-3 border border-white/5">
                  ...
                  <DropdownMenuContent className="bg-gray-900 border-white/10 text-white min-w-[200px]">
                    {fonts.map((f) => (
                      <DropdownMenuItem key={f.value} onClick={() => editor.chain().focus().setFontFamily(f.value).run()} className="hover:bg-white/10 focus:bg-white/10 py-2.5 cursor-pointer" style={{ fontFamily: f.value }}>{f.name}</DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                </DropdownMenu>

                <div className="flex items-center gap-1 bg-white/5 rounded-md p-0.5 border border-white/5">
                  <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} className={`h-7 px-2 text-[10px] font-bold ${editor.isActive('heading', { level: 1 }) ? 'text-blue-500 bg-white/10' : 'text-gray-500'}`}>H1</Button>
                  <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} className={`h-7 px-2 text-[10px] font-bold ${editor.isActive('heading', { level: 2 }) ? 'text-blue-500 bg-white/10' : 'text-gray-500'}`}>H2</Button>
                  <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} className={`h-7 px-2 text-[10px] font-bold ${editor.isActive('heading', { level: 3 }) ? 'text-blue-500 bg-white/10' : 'text-gray-500'}`}>H3</Button>
                </div>
              </div>
              <span className="text-[9px] uppercase font-bold text-gray-600 tracking-tighter mt-1">Typography</span>
            </div>

            <div className="flex flex-col items-center border-r border-white/10 pr-4">
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleBold().run()} className={`w-8 h-8 p-0 ${editor.isActive('bold') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><Bold className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleItalic().run()} className={`w-8 h-8 p-0 ${editor.isActive('italic') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><Italic className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleUnderline().run()} className={`w-8 h-8 p-0 ${editor.isActive('underline') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><UnderlineIcon className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleStrike().run()} className={`w-8 h-8 p-0 ${editor.isActive('strike') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><Strikethrough className="w-4 h-4" /></Button>
                <Separator orientation="vertical" className="h-4 bg-white/10 mx-1" />
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleSubscript().run()} className={`w-8 h-8 p-0 ${editor.isActive('subscript') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><SubscriptIcon className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleSuperscript().run()} className={`w-8 h-8 p-0 ${editor.isActive('superscript') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><SuperscriptIcon className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleCode().run()} className={`w-8 h-8 p-0 ${editor.isActive('code') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><Code className="w-4 h-4" /></Button>
              </div>
              <span className="text-[9px] uppercase font-bold text-gray-600 tracking-tighter mt-1">Font Styles</span>
            </div>

            <div className="flex flex-col items-center">
              <div className="flex items-center gap-1">
                <Popover>
                  <PopoverTrigger asChild><Button variant="ghost" size="sm" className="w-8 h-8 p-0 text-gray-400 hover:text-white"><Palette className="w-4 h-4" /></Button></PopoverTrigger>
                ...
                <Popover>
                  <PopoverTrigger asChild><Button variant="ghost" size="sm" className="w-8 h-8 p-0 text-gray-400 hover:text-white"><Highlighter className="w-4 h-4" /></Button></PopoverTrigger>
                  <PopoverContent className="w-auto p-2 bg-gray-900 border-white/10 flex gap-1 shadow-2xl backdrop-blur-xl">
              ...
              <span className="text-[9px] uppercase font-bold text-gray-600 tracking-tighter mt-1">Colors</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 p-2">
            <div className="flex flex-col items-center border-r border-white/10 pr-4">
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().setTextAlign('left').run()} className={`w-8 h-8 p-0 ${editor.isActive({ textAlign: 'left' }) ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><AlignLeft className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().setTextAlign('center').run()} className={`w-8 h-8 p-0 ${editor.isActive({ textAlign: 'center' }) ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><AlignCenter className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().setTextAlign('right').run()} className={`w-8 h-8 p-0 ${editor.isActive({ textAlign: 'right' }) ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><AlignRight className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().setTextAlign('justify').run()} className={`w-8 h-8 p-0 ${editor.isActive({ textAlign: 'justify' }) ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><AlignJustify className="w-4 h-4" /></Button>
              </div>
              <span className="text-[9px] uppercase font-bold text-gray-600 tracking-tighter mt-1">Alignment</span>
            </div>

            <div className="flex flex-col items-center border-r border-white/10 pr-4">
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleBulletList().run()} className={`w-8 h-8 p-0 ${editor.isActive('bulletList') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><List className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleOrderedList().run()} className={`w-8 h-8 p-0 ${editor.isActive('orderedList') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><ListOrdered className="w-4 h-4" /></Button>
                <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleTaskList().run()} className={`w-8 h-8 p-0 ${editor.isActive('taskList') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><CheckSquare className="w-4 h-4" /></Button>
              </div>
              <span className="text-[9px] uppercase font-bold text-gray-600 tracking-tighter mt-1">Lists</span>
            </div>

            <div className="flex flex-col items-center border-r border-white/10 pr-4">
              <div className="flex items-center gap-2">
                 <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()} className="h-8 gap-2 bg-white/5 text-[10px] text-gray-400 font-bold px-3 border border-white/5"><TableIcon className="w-3.5 h-3.5" /> Table</Button>
                 <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().setHorizontalRule().run()} className="w-8 h-8 p-0 text-gray-400 hover:text-white"><Minus className="w-4 h-4" /></Button>
                 <Button variant="ghost" size="sm" onClick={() => editor.chain().focus().toggleBlockquote().run()} className={`w-8 h-8 p-0 ${editor.isActive('blockquote') ? 'text-blue-500 bg-white/10' : 'text-gray-400'}`}><Quote className="w-4 h-4" /></Button>
              </div>
              <span className="text-[9px] uppercase font-bold text-gray-600 tracking-tighter mt-1">Structure</span>
            </div>

            <div className="flex flex-col items-center ml-auto">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                   <Button variant="ghost" size="sm" className="h-8 gap-2 bg-blue-500 text-white text-[11px] font-bold uppercase tracking-widest px-4 hover:bg-blue-600 shadow-xl" disabled={uploading !== null}>
                    {uploading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Plus className="w-3.5 h-3.5" />}
                    {uploading ? "Uploading..." : "Insert Multimedia"}
                   </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="bg-gray-900 border-white/10 text-white min-w-[220px]">
                  <DropdownMenuItem onClick={() => handleUploadClick("image")} className="gap-3 py-3 cursor-pointer hover:bg-white/5"><ImageIcon className="w-4 h-4 text-blue-400" /> Image File</DropdownMenuItem>
                  <DropdownMenuItem onClick={addYoutubeVideo} className="gap-3 py-3 cursor-pointer hover:bg-white/5"><YoutubeIcon className="w-4 h-4 text-red-500" /> YouTube Video</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleUploadClick("video")} className="gap-3 py-3 cursor-pointer hover:bg-white/5"><Video className="w-4 h-4 text-purple-400" /> Video File</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleUploadClick("audio")} className="gap-3 py-3 cursor-pointer hover:bg-white/5"><Music className="w-4 h-4 text-emerald-400" /> Audio File</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleUploadClick("attachment")} className="gap-3 py-3 cursor-pointer hover:bg-white/5"><Paperclip className="w-4 h-4 text-orange-400" /> Document Attachment</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              <span className="text-[8px] uppercase font-bold text-gray-700 tracking-[0.1em] mt-1">Media Center</span>
            </div>
          </div>
        </div>
      </div>
