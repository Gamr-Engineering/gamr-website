...
const InsightsSection = () => {
    const { allInsights, caseStudies, blogPosts, loading } = useInsights();
    const navigate = useNavigate();
    
    if (loading || allInsights.length === 0) {
        return (
            <section id="case-studies" className="py-32 bg-black flex items-center justify-center min-h-screen">
               <div className="flex flex-col items-center gap-4">
                 <div className="w-10 h-10 rounded-full border-4 border-blue-500 border-t-transparent animate-spin" />
                 <p className="text-gray-400 uppercase tracking-widest text-xs font-bold">Loading Insights...</p>
               </div>
            </section>
        );
    }
